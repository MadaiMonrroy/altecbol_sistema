    ...
    <div class="py-8">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="rounded-2xl p-6 shadow text-[var(--text-main)]"
                ...
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-6">
                    <div>
                        ...
                        <p class="text-sm mt-1 ui-muted">
                            Administra las marcas disponibles en tu catálogo.
                        </p>
                    </div>

                    <div class="flex flex-wrap items-center gap-3">
                        <a href="{{ route('admin.productos.marcas.create') }}" class="ui-btn-primary">
                            + Nueva marca
                        </a>

                        <a href="{{ route('admin.productos.marcas.index') }}" class="ui-btn">
                            Limpiar
                        </a>
                    </div>
                </div>
                ...
                {{-- Filtros --}}
                <form method="GET" action="{{ route('admin.productos.marcas.index') }}"
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            @forelse ($marcas as $marca)
                                @php
                                    $estadoStyle = match ($marca->estado) {
                                        'activo'
                                ...
                                <tr class="ui-table-row transition-colors"
                                    ...
                                    data-search="{{ strtolower(($marca->nombre ?? '') . ' ' . ($marca->descripcion ?? '') . ' ' . ($marca->estado ?? '')) }}">
                                    <td class="px-4 py-3 ui-text">
                                        {{ ($marcas->currentPage() - 1) * $marcas->perPage() + $loop->iteration }}
                                    </td>

                                    <td class="px-4 py-3">
                                        <div class="w-28 h-14 rounded-xl overflow-hidden border"
                                    ...
                                    <td class="px-4 py-3 ui-text">
                                        ...
                                        <div class="text-xs ui-muted mt-1">
                                            {{ optional($marca->created_at)->format('H:i') ?: '' }}
                                        </div>
                                    </td>

                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('admin.productos.marcas.edit', $marca->id) }}"
                                            ...
                                            <form method="POST"
                                                action="{{ route('admin.productos.marcas.destroy', $marca->id) }}"
                            ...
                            @empty
                                <tr id="empty-db-row">
                                    <td colspan="7" class="px-4 py-8 text-center ui-muted">
                                        No hay marcas registradas con esos filtros.
                                    </td>
                                </tr>
                            @endforelse
                            ...
                            <tr id="empty-search-row" style="display:none;">
                                <td colspan="7" class="px-4 py-8 text-center ui-muted">
                                    No hay marcas en esta tabla con esa búsqueda.
                                </td>
                            </tr>
                        </tbody>
                    </table>
